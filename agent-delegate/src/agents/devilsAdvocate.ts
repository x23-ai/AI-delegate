  ...
  async run(ctx): Promise<ChallengeOutput> {
    ...
    async function worker(startIdx: number) {
      for (let i = startIdx; i < targets.length; i += conc) {
        ...
        const ev = await findEvidenceForClaim(ctx, llm, role, premise, ['risk', 'constraint', 'policy conflict', 'opposition'], { seenUris });
    ...
    ctx.trace.addStep({
      ...
      input: { evidenceCount: evidenceByPremise.reduce((n, e) => n + e.docs.length, 0), searchAttempts: attempts.length },
      output: { counterpoints, failureModes, confidence: out.confidence ?? null, uniqueCitations: uniqueCitations.length },
